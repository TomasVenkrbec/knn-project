...
class Dataset:
    ...
    def preview_data(self):
        for i in range(9):
            plt.subplot(330 + 1 + i)
            plt.imshow(self.x_train[random.randint(0, len(self.x_train))], cmap=plt.get_cmap('gray'))
        plt.show()

        for i in range(9):
            plt.subplot(330 + 1 + i)
            plt.imshow(self.x_val[random.randint(0, len(self.x_val))], cmap=plt.get_cmap('gray'))
        plt.show()
